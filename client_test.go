...
func TestUpdateSite(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Origin == nil {
			t.Fatal("expected origin in update request")
		}
		if req.Origin.Port != 8080 {
			t.Errorf("expected port 8080, got %d", req.Origin.Port)
		}
	...
	client := NewClient(server.URL, "cr_testkey")
	site, err := client.UpdateSite("site_abc123", UpdateSiteRequest{
		Origin: &Origin{
			MeshIP:   "10.42.100.1",
			Port:     8080,
			Protocol: "http",
	...
	if site.Origin.Port != 8080 {
		t.Errorf("expected port 8080, got %d", site.Origin.Port)
	}
}
...
func TestNewClientTrimsTrailingSlash(t *testing.T) {
	t.Parallel()
	client := NewClient("https://lighthouse.example.com/", "cr_testkey")
	if client.baseURL != "https://lighthouse.example.com" {
