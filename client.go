...
func (c *Client) GetSite(id string) (*Site, error) {
	httpReq, err := http.NewRequest("GET", c.baseURL+"/v1/sites/"+id, nil)
	if err != nil {
		return nil, fmt.Errorf("create request: %w", err)
	}
	c.setHeaders(httpReq)
...
func (c *Client) UpdateSite(id string, req UpdateSiteRequest) (*Site, error) {
	body, err := json.Marshal(req)
	if err != nil {
		return nil, fmt.Errorf("marshal request: %w", err)
	}

	httpReq, err := http.NewRequest("PATCH", c.baseURL+"/v1/sites/"+id, bytes.NewReader(body))
	if err != nil {
		return nil, fmt.Errorf("create request: %w", err)
	}
	c.setHeaders(httpReq)
